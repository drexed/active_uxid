...
describe ActiveUxid::Hash do
  let(:encoder) { ActiveUxid::Hash.new(10) }
  let(:decoder) { ActiveUxid::Hash.new('q5D8inm0') }

  describe '#encode' do
    it 'to be "q5D8inm0"' do
      expect(ActiveUxid::Hash.encode(10)).to eq('q5D8inm0')
    end
  end

  describe '#decode' do
    it 'to be 10' do
      expect(ActiveUxid::Hash.decode('q5D8inm0')).to eq(10)
    end
  end
